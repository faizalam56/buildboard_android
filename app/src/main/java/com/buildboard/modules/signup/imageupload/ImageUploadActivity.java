...
public class ImageUploadActivity extends AppCompatActivity implements AppConstant {
    ...
    void saveButtonTapped() {
        if (selectedImage == null) {
            ...
            return;
        } else uploadImage(this, prepareFilePart("image", Utils.getImagePath(this, selectedImage)));
    }
    ...
    public void uploadImage(Activity activity, MultipartBody.Part image) {
        ...
        if (AppPreference.getAppPreference(this).getBoolean(IS_CONTRACTOR))
            imageUploadRequest.setType("contractor"); //TODO remove hardcoded strings
        else imageUploadRequest.setType("consumer"); //TODO remove hardcoded strings

        imageUploadRequest.setFileType("image"); //TODO remove hardcoded strings
        imageUploadRequest.setImage(image);

        DataManager.getInstance().uploadImage(activity, imageUploadRequest, new DataManager.DataManagerListener() {
            @Override
